    ...
    XML_HANDLE_T *pXML)
{
    ...
    if (token == TOK_OPEN)
    {
        ...
        if (token == TOK_ID)
        {
            strncpy(pXML->tokenTag, pXML->tokenValue, MAX_TAG_LEN);
    ...
    size_t          bufSize)
{
    ...
    printf("Temporary XML file could not be opened to write\n");
    return RET_IO_ERR;
#endif
}
    ...
    int             maxlen)
{
    ...
    while (ret == 99)
    {
        ...
        else if ((pXML->tagDepth == pXML->tagDepthSeek) && (token == TOK_START_TAG))
        {
            ...
            strncpy(tag, pXML->tokenTag, (UINT32) maxlen);
            ret = 0;
        }
        /* else ignore */
    }
    ...
    CHAR            *value)
{
    ...
    if (token == TOK_ID)
    {
        strncpy(attribute, pXML->tokenValue, MAX_TOK_LEN - 1u);
        token = trdp_XMLNextTokenHl(pXML);
        ...
        if (token == TOK_EQUAL)
        {
            ...
            if (token == TOK_ID)
            {
                ...
                strncpy(value, pXML->tokenValue, MAX_TOK_LEN - 1u);

                uIntValue  = (UINT32) strtoul(value, NULL, 10); /* we expect unsigned values mostly */
                if ((value[0] == '-') || (uIntValue == ULONG_MAX ))
                {
